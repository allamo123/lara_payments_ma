...
class HyperPayPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->hyperpay_url = config('ma-payments.HYPERPAY_URL');
        $this->hyperpay_base_url = config('ma-payments.HYPERPAY_BASE_URL');
        $this->hyperpay_token = config('ma-payments.HYPERPAY_TOKEN');
        $this->currency = config('ma-payments.HYPERPAY_CURRENCY');
        $this->hyperpay_credit_id = config('ma-payments.HYPERPAY_CREDIT_ID');
        $this->hyperpay_mada_id = config('ma-payments.HYPERPAY_MADA_ID');
        $this->hyperpay_apple_id = config('ma-payments.HYPERPAY_APPLE_ID');
        $this->app_name = config('ma-payments.APP_NAME');
    ...
    public function verify(Request $request)
    {
        ...
        if (in_array($final_result["result"]["code"], ["000.000.000", "000.100.110", "000.100.111", "000.100.112"])) {
            ...
            return [
                'success' => false,
                'payment_id'=>$request['id'],
                'message' => __('ma::messages.PAYMENT_FAILED_WITH_CODE', ['CODE' => $final_result["result"]["code"]]),
                'process_data' => $final_result
            ];
        }
    }
    ...
    {
        return view('ma::html.hyper_pay', ['model' => $this, 'brand' => $this->getBrand()])->render();
    }
