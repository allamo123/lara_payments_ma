...
class PayPalPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paypal_client_id = config('ma-payments.PAYPAL_CLIENT_ID');
        $this->paypal_secret = config('ma-payments.PAYPAL_SECRET');
        $this->verify_route_name = config('ma-payments.VERIFY_ROUTE_NAME');
        $this->paypal_mode = config('ma-payments.PAYPAL_MODE');
        $this->currency = config('ma-payments.PAYPAL_CURRENCY');
    }
