...
class PaymobPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paymob_api_key = config('ma-payments.PAYMOB_API_KEY');
        $this->paymob_integration_id = config('ma-payments.PAYMOB_INTEGRATION_ID');
        $this->paymob_iframe_id = config("ma-payments.PAYMOB_IFRAME_ID");
        $this->currency = config("ma-payments.PAYMOB_CURRENCY");
    }
    ...
    public function verify(Request $request): array
    {
        ...
        if (hash_hmac('sha512', $string, config('ma-payments.PAYMOB_HMAC'))) {
            if ($request['success'] == "true") {
                ...
                return [
                    'success' => false,
                    'payment_id'=>$request['order'],
                    'message' => __('ma::messages.PAYMENT_FAILED_WITH_CODE',['CODE'=>$this->getErrorMessage($request['txn_response_code'])]),
                    'process_data' => $request->all()
                ];
            }
    ...
    public function getErrorMessage($code){
        $errors=[
            'BLOCKED'=>__('ma::messages.Process_Has_Been_Blocked_From_System'),
            'B'=>__('ma::messages.Process_Has_Been_Blocked_From_System'),
            '5'=>__('ma::messages.Balance_is_not_enough'),
            'F'=>__('ma::messages.Your_card_is_not_authorized_with_3D_secure'),
            '7'=>__('ma::messages.Incorrect_card_expiration_date'),
            '2'=>__('ma::messages.Declined'),
            '6051'=>__('ma::messages.Balance_is_not_enough'),
            '637'=>__('ma::messages.The_OTP_number_was_entered_incorrectly'),
            '11'=>__('ma::messages.Security_checks_are_not_passed_by_the_system'),
        ];
